...
mask1 = Lambda(lambda x : x[:,0,:,:]/(x[:,0,:,:]+x[:,1,:,:]))(mask) ;print("mask 1 ", mask1.shape)
mask2 = Lambda(lambda x : x[:,1,:,:]/(x[:,0,:,:]+x[:,1,:,:]))(mask) ;print("mask 2 ", mask2.shape)
...
tb = TensorBoard(log_dir='./AV_100_maskeq1/tb_logs', histogram_freq=0, batch_size= batch_size,
...
try:
    ...
    pass
AV.save(model_dir + '/AV_100_maskeq1.h5')
...
# save json
with open(history_dir + '/history_100_maskeq1.json', 'w') as fp:
    r = json.dump(history.history, fp, indent=2)

# read json
with open(history_dir + '/history_100_maskeq1.json', 'r') as fp:
