...
faces1_folder = '../../../200_download/separated_data/faces1'
...
for path in paths:  
  ...
  count += 1
  for i in range(75):
   im = cv2.imread('../../../200_download/separated_data/faces1/'+path+'/cropped'+str(i)+'.jpg', cv2.IMREAD_UNCHANGED)
   print(im.shape)
   size = (int(64), int(64))
   ...
   try:
       os.makedirs('../../../200_download/separated_data/faces2/'+path)
   except FileExistsError:
       pass
   
   
   
   cv2.imwrite( '../../../200_download/separated_data/faces2/'+path+'/face'+str(i)+'.jpg', resized);
  # waitKey(0);
